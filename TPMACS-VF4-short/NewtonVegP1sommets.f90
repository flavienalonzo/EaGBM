SUBROUTINE NewtonVegP1sommets(A,Vold, V,U,Ue,Nut, ndim, choixf,  temps)
    !--------
    ! Modules
    !--------
    USE longr
    ...
    REAL(kind=long), dimension (ndim), intent(in)    :: Vold, U, Ue, Nut
    ...
    DO kiter = 1, kitermax
       ...
       DO i = 1, NbInc
          FF(i) = AireDsommet(i)* ( Xk(i)-Vold(i) )/dt - AireDsommet(i)*(- VEGF_degr*Xk(i) &
          & -VEGF_cons*Ue(i)*Xk(i) )
          if (Nut(i)<=seuil_hypo.and.Nut(i)>seuil_necro) then
            FF(i) = FF(i) - AireDsommet(i)*VEGF_prod*U(i)
          end if
          !! derivee de FF par rapport a Xk
          call ajout(i,i, AireDSommet(i)/dt, A )
          call ajout(i,i, AireDsommet(i)*(VEGF_cons*Ue(i) + VEGF_degr), A ) 
       END DO
       !-----------------------------------------
  ...
  END SUBROUTINE NewtonVegP1sommets