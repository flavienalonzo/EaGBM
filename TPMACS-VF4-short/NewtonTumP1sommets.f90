SUBROUTINE NewtonTumP1sommets(A, F, Uold, U, S, AA, ndim, choixf, temps)
    !--------
    ! Modules
    !--------
    USE longr
    ...
    DO kiter = 1, kitermax
       ...
       DO i = 1, NbInc
          FF(i) = AireDsommet(i)*( Xk(i)-Uold(i) )/dt -AireDsommet(i)*(- apop*Xk(i)-chemotherapy(temps)*Xk(i))
          if (isbomba(i).eqv..true.) then 
            FF(i) = FF(i) + AireDsommet(i)*radiotherapy(temps)*Xk(i)
            call ajout(i,i, AireDSommet(i)*radiotherapy(temps),A)
          end if
          if (S(i)>seuil_hypo) then
            FF(i) = FF(i) -AireDsommet(i)*rate*Xk(i)*(1-AA(i))
            call ajout(i,i, - AireDsommet(i)*rate*(1-AA(i)), A )
          end if
          !! derivee de FF par rapport a Xk
          call ajout(i,i, AireDSommet(i)/dt, A )
          
          call ajout(i,i, AireDSommet(i)*apop+AireDSommet(i)*chemotherapy(temps),A)
       END DO
  ...
  END SUBROUTINE NewtonTumP1sommets
