SUBROUTINE   maillage
  ...
  IF (maillageregulier== 1) then 
     ...
     ! affecter  les coordonn�es de chaque sommet
     ! affecter  les centres de Voronoi de chaque triangle

     Nbt = Nx*Ny
     ! les rectnagles sont numerot� de i=1,Nx, j=1,NY 
     ! le numero dur rectangle est kt = i+(j-1)*Nx
     ! Les sommets sont numerot�s de i=1,Nx+1; j=1, NY+1
     ...
     DO i=1,Nx
        DO j=1,NY
           ...
           ! Les mailles en dehors de la geometrie ne sont pas mis � -1
        END DO
     END DO

     ! ------------------------------------------------------------
     ! -- Les coordonn�es de chaque sommet et le type du sommet
     ! ------------------------------------------------------------ 
     NbS = (Nx+1)*(Ny+1)
     ...
     ! ------------------------------------------------ 
     ! -- coordonn�es du centroide de chaque maille  
     ! ------------------------------------------------ 
  ...
  ELSE 
     ...
     If (Maillage_Format == 1) then 
        ...
        !====================
        ! ---------------------------------------------------------------- 
        ! -- lecture des coordonn�es du centroide de chaque triangle  
    ...
    !-----  !!!!!!!!!!!!!!!!!!!!!
     IF (Maillage_Format == 2) THEN
        ...
        close(uvnode)
        !! le type du sommet n'est pas utilis� pour l'instant
