...
Contains
  ...
  function muKL(y)
    ...
    select case(ChoixChi)
    case(3)
      muKL = CoefTranspChi*y*(1-y)/CoefdiffuAdeg
    case(4)
  ...
  function DerivMuKLCroit(y)
   ...
   ! integer :: choix
    ...
    case(3)
      umax= 0.5D0
       if (y < umax) then
         DerivMuKLCroit = CoefTranspChi *(1.D0-2*y)/CoefdiffuAdeg
      else
         DerivMuKLCroit = 0.D0
      end if
    case(4)
 ...
 !=======================
  function MuKLDeCroit(y)
    ...
    case(3)
      ...
      z=max(y,umax)
      if (y>umax) then
         MuKLDeCroit = CoefTranspChi*(z*(1-z)-umax*(1-umax))/CoefdiffuAdeg
      else 
         MuKLDeCroit = 0.D0
      end if 
    case(4)
  ...
  function DerivMuKLDecroit(y)
    !=======================
    USE longr
    REAL( kind=long) :: y, z,DerivMuKLDecroit, umax
    !
    ...
    case(3)
      ...
      z=max(y,umax)
      if (y>umax) then
         DerivMuKLDecroit = CoefTranspChi*(1.D0-2*y)/CoefdiffuAdeg
      else 
  ...
  function chemotherapy(y)
   ...
   integer :: i 
   chemotherapy = 0.D0
   if (y>=start_chemo.and.y<=end_chemo) then
   ...
   function radiotherapy(y)
      ...
      implicit none 
      real(kind=long), intent(in) :: y 
      real(kind=long) :: radiotherapy, taufrac, deltataufrac, muradio,gfrac,hfrac,phifrac
      integer :: i,Nfrac
      radiotherapy = 0.D0
      Nfrac=30; taufrac = 5/86400.D0; deltataufrac = 25/86400.D0; muradio = 24*0.46D0;
      phifrac = EXP(-muradio*(taufrac+deltataufrac))
      gfrac = 2*(muradio*taufrac-1+EXP(-muradio*taufrac))/(muradio*taufrac)**2
      hfrac = 2*(Nfrac*phifrac-Nfrac*phifrac**2-phifrac+phifrac**(Nfrac+1))/(Nfrac*(1-phifrac)**2)
      do i=1,Nradio
         if (y>=radio_time(1,i).and.radio_time(2,i)> y) then
            radiotherapy = radio_alpha*Nfrac*dose_radio+radio_beta*Nfrac*dose_radio**2*(&
            &gfrac+2*(COSH(muradio*taufrac)-1)/(muradio*taufrac)**2*hfrac)
            !1-EXP(-(radio_alpha*dose_radio+radio_beta*dose_radio**2))
            !print*,radiotherapy
         end if
      end do
      if (y>=start_chemo.and.56> end_chemo) then
         radiotherapy = Upsilon*radiotherapy
      end if 
      
   end function radiotherapy

   function zone_bomba(Tumeur) result(bisbomba)
      use longr 
      use parmmage
      real(kind=long), dimension(NbInc), intent(in) :: Tumeur
      logical, dimension(NbInc) :: bisbomba
      integer :: i,j
      do i=1,NbInc
         if (Tumeur(i)>=0.5*seuil_surg) then
            bisbomba(i) = .true.
         else
            bisbomba(i) = .false.
         end if
      ...
      do i=1,Nbt
         ...
         select case(ntypt(i))
         case(300,400,500)
            bisbomba(NuSoK(j,i)) = .true.
         end select 
         end do
      end do
   end function zone_bomba
