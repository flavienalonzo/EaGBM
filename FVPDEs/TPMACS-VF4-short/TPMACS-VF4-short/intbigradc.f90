...
!!$       write(6,*)'seuil=',seuil , 'tolerence = ', tolerence   
    END DO
    !print*,'nbigr =', nbigr, 'seuil bigrad=',seuil
    !------------
    ! Impressions
    !------------
    !   if (iprint >= 3) then
...
function BiCGSTAB(Tab_A,B,nbr_enty,dim,tolerence) result(Tab_X)
implicit none
integer, INTENT(IN)   :: nbr_enty, dim 
type(MatCreux), dimension(nbr_enty,nbr_enty), intent(in) :: Tab_A
real(kind=long), dimension(nbr_enty*dim), intent(in) :: B
real(kind=long), intent(in) :: tolerence
real(kind=long), dimension(nbr_enty*dim) :: Tab_X

real(kind=long) :: rho, alpha, omega , beta
real(kind=long), dimension(nbr_enty*dim) :: r0,r, v, p, s, t, g
integer :: n,k,h,n_max

n= 0 ; n_max =200*dim + 500
...
do k=1,nbr_enty
  do h=1,nbr_enty
    r(1+(k-1)*dim : k*dim) = r(1+(k-1)*dim : k*dim) - Tab_A(k,h)*Tab_X(1+(h-1)*dim:h*dim)
  end do
end do
do while(n<=n_max)
  ...
  do k=1,nbr_enty
    do h=1,nbr_enty
      v(1+(k-1)*dim:k*dim) = v(1+(k-1)*dim:k*dim) + Tab_A(k,h)*p(1+(h-1)*dim:h*dim)
    end do
  end do
  ...
  do k=1,nbr_enty
    do h=1,nbr_enty
      t(1+(k-1)*dim:k*dim) = t(1+(k-1)*dim:k*dim)+Tab_A(k,h)*s(1+(h-1)*dim:h*dim)
    end do
  end do
